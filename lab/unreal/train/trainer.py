...
class Trainer(object):
  def __init__(self,
    ...
    self.episode = 0
    self.level_seed = np.random.randint(100000)
    self.episode_reward = 0
    # For log output
    self.prev_local_t = 0

  def prepare(self):
    self.environment = Environment.create_environment(self.episode,self.level_seed)
  ...
  def _fill_experience(self, sess):
    ...
    if terminal:
        ...
        old_seed = self.level_seed
        self.episode += 1
        self.level_seed = np.random.randint(100000)
        self.environment.reset(old_episode,old_seed,self.episode,self.level_seed)
