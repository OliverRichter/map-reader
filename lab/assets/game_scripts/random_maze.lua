...
function api:start(episode, seed, params)
  random.seed(seed)
  local maze_size = math.min(math.floor(episode / 40) * 2 + 7, 21)
  local rows, cols = maze_size,maze_size
  ...
  local st = getRandomEvenCoodinate(rows, cols)
  local count = 0
  ...
  while math.abs(goal[1]-st[1])+1 < rows / 2 and math.abs(goal[2]-st[2])<cols / 2 do
	st = getRandomEvenCoodinate(rows, cols)
    count = count + 1
    if count == 100 then
      print('Problem generating maze')
      break
    end
  end
  maze:setEntityCell(st[1], st[2], 'P')
  print(maze:entityLayer())
  io.flush()
  local map = maze:entityLayer()
